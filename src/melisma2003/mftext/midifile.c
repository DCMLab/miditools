...
int readtrack()		 /* read a track chunk */
{
        ...
        /* This function reads through the statements in the file - Davy */
	...
	if ( Mf_trackstart )
        ...
        /* Here's where we read through the statements in the file - Davy */
	...
	while ( Mf_toberead > 0 ) {
		...
		/* Note statements should be found here - Davy */
...
static
void chanmessage(status,c1,c2)
int status;
int c1, c2;

/* The function "Mf_noteon" has been defined elsewhere as "txt_noteon". If the incoming statement is 
   a 0x80 type, it calls the function "txt_noteon" and thus prints out the appropriate "note_on"
   statement. Similarly for "noteoff"... -Davy */
{
	int chan = status & 0xf;
